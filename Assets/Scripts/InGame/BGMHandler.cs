...
public class BGMHandler : BMSObjectHandlerMultiThread 
{
    ...
    protected override void OnTimeElapsed(double elapsedMilliseconds)
    {
        ...
        if (currentBGMIndex < InGameManager.Instance.patternData.bgmKeySoundChannel.Count && InGameManager.Instance.patternData.bgmKeySoundChannel[currentBGMIndex].Timing <= elapsedMilliseconds)
        {
        ...
        {
            if (InGameManager.Instance.patternData.lines[i].NoteList.Count > 0 && InGameManager.Instance.patternData.lines[i].NoteList[0].Timing <= elapsedMilliseconds)
            {
